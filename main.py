...
vars = {
    ...
    "contact_info": "Site is not affiliated with Burnside High School | Copyright 2026 - Chris Fung",
}
...
def directory(name):
    ...
    if ";" in name or '"' in name or "'" in name:
        print(
            f"[{time}]{RED}[ERROR]: Dangerous characters found in request; cancelling request{RESET}"
        )
        abort(403)
    ...
    return render_template(
        "directory.html", vars=vars, title=name, sites=sites, folders=folders, back=back
    )
...
def force_error(code):
    time = datetime.now().strftime("%H:%M:%S")
    print(
        f'[{time}]{YELLOW}[DEBUG]{RESET}:{RED} Returning forced error code: "{BLUE}{code}{RESET}"'
